...
def run_single_performance_test(
    ...
    if use_treequeue:
        queue_data = TreeQueue(data, maxsize=10)
    else:
        queue_data = mp.Queue(maxsize=10)
...
def run_benchmark_multiprocessing():
    ...
    fig, (axe_1, axe_2) = plt.subplots(1, 2, figsize=(8, 4.5), sharey='all')

    experiences = {
        'Queue': {
            '1 process': {'num_processes': 1, 'color': 'r', 'linestyle': '-', 'axe': axe_1},
            '3 processes': {'num_processes': 3, 'color': 'r', 'linestyle': '-', 'axe': axe_2},
        },
        'TreeQueue': {
            '1 process': {'num_processes': 1, 'color': 'k', 'linestyle': '-', 'axe': axe_1},
            '3 processes': {'num_processes': 3, 'color': 'k', 'linestyle': '-', 'axe': axe_2},
        },
    }
